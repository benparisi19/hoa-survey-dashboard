...
-- Property access requests - users can see their own requests, admins see all
...
CREATE POLICY "property_access_requests_access" ON property_access_requests
  FOR SELECT USING (
    ...
    OR
    -- Users can see their own access requests  
    requester_id IN (
      SELECT person_id FROM people 
      WHERE auth_user_id = auth.uid()
    )
...
-- 3. Added policies for property_invitations and property_access_requests
-- 4. Proper column references based on actual supabase.ts schema
